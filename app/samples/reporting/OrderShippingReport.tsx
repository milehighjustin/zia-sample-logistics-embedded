...
export default function OrderShippingReport(props: {tag: string}) {
    ...
    const getToday = () => {
        ...
        setTimeout(async () => {
          ...
          const today = `${year}-${month}-${day}`
            const response = await ziaBackendCall(`ops/orderShippingReport?tag=${props.tag}&startDate=${today}&endDate=${today}&format=json&status=any`, 'GET', undefined)
    ...
    const getWtd = () => {
        ...
        setTimeout(async () => {
            ...
            const response = await ziaBackendCall(`ops/orderShippingReport?tag=${props.tag}&startDate=${startDate}&endDate=${today}&format=json&status=any`, 'GET', undefined)
            setData(response?.data);
            setLoading(false)
        }, 1)
    }
    ...
    const getRange = () => {
            ...
            if (startDate && endDate) {
                setLoading(true)
                setTimeout(async () => {
                    const response = await ziaBackendCall(`ops/orderShippingReport?tag=${props.tag}&startDate=${startDate}&endDate=${endDate}&format=json&status=any`, 'GET', undefined)
                    setData(response?.data);
                    setLoading(false)
                }, 1)
            }
